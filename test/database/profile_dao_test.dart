...
void main() {
  ...
  group('ProfileDao - CRUD', () {
    ...
    test('filters by visibility', () async {
      await db.profileDao.insertProfile(
        makeProfile(id: 'p1', visibility: 'visible'),
      );
      await db.profileDao.insertProfile(
        makeProfile(id: 'p2', visibility: 'hidden'),
      );
      await db.profileDao.insertProfile(
        makeProfile(id: 'p3', visibility: 'deleted'),
      );

      final visible = await db.profileDao.getAllProfiles(visibility: 'visible');
    ...
    test('counts profiles by visibility', () async {
      ...
      await db.profileDao.insertProfile(
        makeProfile(id: 'p2', visibility: 'visible'),
      );
      await db.profileDao.insertProfile(
        makeProfile(id: 'p3', visibility: 'hidden'),
      );

      final total = await db.profileDao.countProfiles();
      expect(total, 3);

      final visibleCount = await db.profileDao.countProfiles(
        visibility: 'visible',
      );
  ...
  group('ProfileDao - Watch', () {
    test('watches profile changes', () async {
      final stream = db.profileDao.watchAllProfiles(visibility: 'visible');
      await db.profileDao.insertProfile(makeProfile(id: 'p1'));
