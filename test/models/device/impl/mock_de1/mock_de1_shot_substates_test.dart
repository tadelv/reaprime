...
void main() {
  group('MockDe1 shot substates', () {
    ...
    test('emits preinfusion when profileFrame < targetVolumeCountStart',
        () async {
      final profile = _testProfile(
        targetVolumeCountStart: 2,
        steps: [
          ProfileStepPressure(
            name: 'fill', pressure: 2.0, seconds: 1, temperature: 92,
            sensor: TemperatureSensor.coffee, transition: TransitionType.fast,
            volume: 0,
          ),
          ProfileStepPressure(
            name: 'soak', pressure: 2.0, seconds: 1, temperature: 92,
            sensor: TemperatureSensor.coffee, transition: TransitionType.fast,
            volume: 0,
          ),
          ProfileStepPressure(
            name: 'pour', pressure: 9.0, seconds: 3, temperature: 94,
            sensor: TemperatureSensor.coffee, transition: TransitionType.fast,
            volume: 0,
          ),
        ],
      ...
      final sub = machine.currentSnapshot.listen((s) {
        ...
        if (ss == MachineSubstate.preinfusion || ss == MachineSubstate.pouring) {
    ...
    test('emits pouring when profileFrame >= targetVolumeCountStart',
        () async {
    ...
    test('emits pouringDone and idle at shot end', () async {
      final profile = _testProfile(
        ...
        steps: [
          ProfileStepPressure(
            name: 'quick', pressure: 9.0, seconds: 1, temperature: 94,
            sensor: TemperatureSensor.coffee, transition: TransitionType.fast,
            volume: 0,
          ),
        ],
      ...
      final hasPouringDone =
          snapshots.any((s) => s.state.substate == MachineSubstate.pouringDone);
    ...
    test('profileFrame in snapshot matches current step index', () async {
      final profile = _testProfile(
        ...
        steps: [
          ProfileStepPressure(
            name: 'step0', pressure: 2.0, seconds: 1, temperature: 92,
            sensor: TemperatureSensor.coffee, transition: TransitionType.fast,
            volume: 0,
          ),
          ProfileStepPressure(
            name: 'step1', pressure: 9.0, seconds: 3, temperature: 94,
            sensor: TemperatureSensor.coffee, transition: TransitionType.fast,
            volume: 0,
          ),
        ],
