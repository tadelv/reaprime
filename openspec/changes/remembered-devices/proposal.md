...
- **Known limitation:** matching is by `deviceId`. BLE (MAC), WiFi (`wifi:<host>`), and DE1-USB ids are stable; a USB Half Decent Scale on macOS has an unstable id (libserialport reports null vid/pid → port-address fallback), so a remembered USB-HDS may show a stale unavailable entry alongside a fresh available one. Documented; most devices are unaffected.
