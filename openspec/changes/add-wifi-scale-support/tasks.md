...
- [x] 8.2 Update `doc/Api.md` for the manual WiFi endpoint REST API (task 6.2). (Discovered WiFi scales need no API change — they flow through the existing device streams — but manual-IP entry DOES require the new `/api/v1/devices/wifi` endpoints so the skin can drive `addManualEndpoint`/`removeManualEndpoint`.)
