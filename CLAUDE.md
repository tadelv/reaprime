...
- **Preferred Device Selection:**
  - Store preferred DE1 and Scale device IDs in application preferences (using `SettingsController`/`shared_preferences`)
  - On app startup, after device scan, automatically connect to preferred devices if found
  - Ignore other devices even if multiple are discovered
  - Provide UI in Settings to set/clear preferred devices
  - Implementation considerations:
    - Add `preferredDe1Id` and `preferredScaleId` to `SettingsController`
    - Modify `DeviceController.scanForDevices()` or add post-scan filter
    - Update `_handleScan()` in `SettingsTile` to check preferences before showing dialog
    - Add "Set as Preferred" option in device selection dialog

### WebUI Features

- **Embedded WebView:** Add embedded webview to `LandingFeature` instead of launching external browser
  - Consider using `webview_flutter` or similar package
  - Allow users to interact with WebUI directly within the app
  - Provide option to open in external browser as fallback

### Responsive Design Improvements

#### Lower Priority UI Polish

- **LandingFeature responsive improvements:**
  - Adjust card sizes and spacing for very small screens (< 400px width)
  - Consider collapsible skin details for mobile to reduce vertical scroll
  
- **History feature responsiveness:**
  - Review `history_feature.dart` for table/chart overflow on narrow screens
  - Consider horizontal scroll for wide data tables on mobile
  
- **Realtime shot feature:**
  - Review `realtime_shot_feature.dart` for chart responsiveness
  - Ensure fl_chart graphs scale appropriately on different screen sizes
  
- **Settings view polish:**
  - Consider using `ExpansionTile` for grouped settings on mobile
  - Add section headers for better organization on small screens
  
- **Tiles responsiveness:**
  - Review all tiles in `home_feature/tiles/` for content overflow
  - Consider responsive font sizes using MediaQuery
  
- **String interpolation cleanup:**
  - Remove unnecessary braces in string interpolations (lint warnings)
