...
- Decenza and despresso decode Bookoo weight from bytes 7-9 after header `03 0B`; despresso uses the same six-byte tare and timer commands as ReaPrime.
...
- Bookoo accepts only `03 0B` packets with the complete ten-byte weight body; battery remains optional.
...
- Keep Bookoo command bytes and current write behavior because issue #53 supplies no contrary packet-level evidence.
