...
Sync accepts `target` (URL), `mode` (`pull`, `push`, `two_way`), `onConflict` (`skip` or `overwrite`), and `sections` (profiles, shots, workflow, settings, store, steams, beans, grinders). `sections` is optional for every mode; omission means all locally registered sections. An explicitly empty list is invalid. Unknown sections return `400`; duplicate names are deduplicated in first-occurrence order.
