...
| GET | `/api/v1/machine/cupWarmer` | Read cup-warmer setpoint °C + live mat temperature (`currentTemperature`, `null` = no valid reading / older FW) and the FW-owned scheduled pre-warm (`prewarmEnabled`, `prewarmLeadMinutes` 0–120, read-only `prewarmActive`; all three `null` on FW without the registers) — render a placeholder for `null`, never fake data — Bengle only, 404 elsewhere | |
| PUT | `/api/v1/machine/cupWarmer` | Set cup-warmer setpoint °C (range 0.0–80.0; `> 0` = on, `0.0` = off; the FW enable register is app-managed — cleared on machine reboot, re-asserted on reconnect) and/or the scheduled pre-warm (`prewarmEnabled`, `prewarmLeadMinutes` 0–120, persisted in FW; the pair is written together, an omitted half keeps the machine's current value). At least one of the three fields is required (empty body → 400). `prewarmActive` is read-only and ignored. The 200 body echoes the pre-warm state read back from the machine (`null` = FW without the registers — the write was silently inert) — Bengle only | |
